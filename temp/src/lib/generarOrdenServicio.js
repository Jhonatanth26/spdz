...
// Sube bytes generados por el propio sistema (ej. un PDF armado con pdf-lib), no un
// archivo elegido por el usuario. Devuelve la ruta guardada, igual que subirArchivo().
export async function subirBytes(bytes, nombreArchivo, carpeta = 'general', contentType = 'application/pdf') {
  const nombreLimpio = nombreArchivo.replace(/[^a-zA-Z0-9._-]/g, '_')
  const ruta = `${carpeta}/${Date.now()}_${nombreLimpio}`

  const { error } = await supabase.storage.from('adjuntos').upload(ruta, bytes, { upsert: false, contentType })
  if (error) {
    console.error('Error subiendo archivo generado a Storage:', error.message)
    return null
  }
  return ruta
}
